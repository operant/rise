...
interact_human = {
	...
	if (([_player] call player_cop) && (not([_target] call player_cop) || ischief)) exitWith {
		if (not(createDialog "civmenu")) exitWith { hint "Dialog Error! Return to lobby and join again!";};
		true;
	};
	if (([_player] call player_opfor) && (not([_target] call player_opfor) || ischief)) exitWith {
		if (not(createDialog "civmenu_opf")) exitWith { hint "Dialog Error! Return to lobby and join again!";};
		true;
	};
	...
	if (([_player] call player_civilian) ||([_player] call player_insurgent)) exitWith {
		if (!(createDialog "civinteraktion")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
		true
	};
	false
};
...
interact_ai = {
	...
	*/
	...
	if((_target in hit_shop_list)) exitWith {
		if !(ishit) exitWith {
			hint "You cannot access Hitman Shops: You are not whitelisted!";
			false
		};
		[_id] call shop_open_dialog;
		true
...
interact_civilian_camera_menu = {_this spawn {
	if (!(createDialog "civcamdialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_civilian_camera = {_this spawn {
	...
	if (not(createDialog "MainCamDialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_warrants_menu = { _this spawn {
	if (!(createDialog "wantedrelease")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_mobile_use = {
	if (!(createDialog "handydialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_atm_menu = { _this spawn {
	...
	if (!(createDialog "bank")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_inventory_menu = {
	...
	_player = _this select 0;
	if (not([_player] call player_human)) exitWith {};
	if (!(createDialog "inventar")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
interact_ticket_receive = { _this spawn {
	...
	response = false;
	if (!(createDialog "ja_nein")) exitWith {hint "Dialog Error! Return to lobby and join again!"};
...
interact_generic_storage_menu = { _this spawn {
	...
	if (!(createDialog "itemkaufdialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
