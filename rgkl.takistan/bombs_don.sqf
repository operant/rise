    ...
    while {true} do {
        ...
        for [{_i=0}, {_i < (count INV_ServerBombArray)}, {_i=_i+1}] do {
            ...
            switch (_name) do {
                ...
                case "geschwindigkeitsbombe_don": {
...
if (_art == "use") then {
    ...
    if (_item == "zeitzuenderbombe_don") then {
        if (!(createDialog "timebombconfig")) exitWith {
            hint "Dialog Error! Return to lobby and join again!";
        };
        sliderSetRange [1, zeitbombe_mintime, zeitbombe_maxtime];
        sliderSetSpeed [1, 1, 10];
        buttonSetAction [4, "[""plant"", ""zeitzuenderbombe_don"", sliderPosition 1] spawn A_SCRIPT_BOMBS; closedialog 0;"];
    ...
    if (_item == "aktivierungsbombe_don") then {
        ["plant", "aktivierungsbombe_don"] spawn A_SCRIPT_BOMBS;
    };
            
    if (_item == "geschwindigkeitsbombe_don") then {
        ...
        buttonSetAction [4, "[""plant"", ""geschwindigkeitsbombe_don"", sliderPosition 1, sliderPosition 8] spawn A_SCRIPT_BOMBS; closedialog 0;"];
    ...
    if (_item == "fernzuenderbombe_don") then {
        ["plant", "fernzuenderbombe_don"] spawn A_SCRIPT_BOMBS;
    };
            
    if (_item == "fernzuender") then {
        if (!(createDialog "remotebomb_remote")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
    };
};
