...
while {true} do {
	...
	if (convoytruck distance copbase1 < 150) exitwith 
		{   
		"if (iscop) then {[player, govconvoybonus] call transaction_bank; player sidechat format[""you received $%1 for the successfully escorting the convoy"", govconvoybonus];};" call broadcast;
		_sidewon = "Cops";
		};
