	...
	if ([_target] call name_tags_vehicle && _distance < 25) exitWith {
		...
		if (_distance < 5 && not(_inside_vehicle) && not(_is_box)) then {
			...
			if (_owner && (locked _target)) exitWith {
				_control ctrlSetStructuredText parseText format["<t size='1.2' font='Zeppelin33Italic' color='#00ff00'>Unlock (Press L)</t><br /><t size='1.2' font='Zeppelin33Italic' color='#ffffff'>Trunk (T)</t>"];
				_handled = true;
			};
