...
if (_art == "Einstellungen") then {
	if (!(createDialog "MainMenu")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
};
...
if (_art == "TextEinstellungen") then {
	if (_zusatzString == "oeffnen") then {
		if (!(createDialog "TextEinstellungenDialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
...
if (_art == "Tutorial") then {
	if (!(createDialog "TutorialDialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
	ctrlSetText[1, localize "STRS_mainmenu_MissionTutorial"];
};

if (_art == "FAQ") then {
	if (!(createDialog "FAQDialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
	ctrlSetText[1, localize "STRS_mainmenu_FAQ"];
};

if (_art == "About") then {
	if (!(createDialog "AboutDialog")) exitWith {hint "Dialog Error! Return to lobby and join again!";};
