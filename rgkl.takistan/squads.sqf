...
if(_art == "create") then {
	...
	if ((_text call ISSE_str_Length) > 30 or (_text call ISSE_str_Length) < 3) exitWith {
...
if(_art == "clientloop") then {
	...
	while {iscop} do {
		...
		for "_c" from 0 to (count squadareas - 1) do {
			...
			_offset = [_squadarea] call flag_squad_get_offset;
			if (!isNil "_offset") then
			{
				if(player distance _squadarea < 10 and (_offset select 2) < 0 and _control == (call INV_mysquad))then {
					_new_offset = [_squadarea, [0,0,0.1]] call flag_squad_set_offset;
					if ((_new_offset select 2) > 0) then {
						[_squadarea, [0,0,0]] call flag_squad_reset_offset;
					};
				};
			};
		};
